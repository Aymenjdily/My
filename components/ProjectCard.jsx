...
const ProjectCard = ({data : {slug, mainImage, projectName, url}, index}) => {
  return (
    <Link href={`${url ? url : "#"}`}>
      <div
        className={`relative h-[50vh] w-full duration-200 hover:opacity-80 rounded-xl shadow-lg cursor-pointer`}
      >
          <Image src={urlFor(mainImage).url()} fill alt="project-Image" className='object-cover' />
          <div className={`absolute rounded-xl w-full h-full bg-black/30`}>
